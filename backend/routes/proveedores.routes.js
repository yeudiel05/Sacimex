...
router.post('/pagos', verificarToken, upload.single('comprobante'), (req, res) => {
  ...
  db.query('INSERT INTO pagos_a_proveedores (id_proveedor, id_usuario_solicita, id_usuario_autoriza, monto_pago, concepto, num_factura_ref, url_comprobante_pago, estatus) VALUES (?, ?, ?, ?, ?, ?, ?, "PAGADO")', 
  [id_proveedor, req.usuario.id, req.usuario.id, monto_pago, concepto, num_factura_ref, url], (err) => {
      if (err) return res.status(500).json({ success: false });
      res.json({ success: true });
  });
});
